...
| **M51** | ✅ | ✅ | 🚧 | 🚧 | ➖ | ⬜ | designed → backend in progress — service identities ([D-ServiceIdentities](architecture/roadmap-decisions.md#d-serviceidentities--machine-clients-authenticate-via-idp-client-credentials-and-resolve-to-service-principals-extends-d-hermenea-l-authzonly), **amended**: per-principal grants `(permission_code, org_id NULL)` replace "role assignments" — instance-scope codes are admin-only via `pdp.go`, so a role could never carry `import.manage`; [north star](architecture/north-star.md) plane 2). Designed in [identity-federation.md](modules/identity-federation.md#service-principals-m51). UI `➖` (principal admin console lands with M52) |
