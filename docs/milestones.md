...
- **Boundary:** a principal has **no reach**. It gets no authority snapshot and no RLS connection
  ...
  organization-owned data (scraped units, memberships, clergy, university staff) is **M53 /
  D-ConnectorPlane**; the `org_id` column and its enforcement ship here so M53 retrofits nothing.
