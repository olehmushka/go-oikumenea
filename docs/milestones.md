...
| **M38** | Criminal/arrest/court records | one `person_legal_records` Object (`pii:special`, GDPR Art. 10, envelope-encrypted offence detail) with a **mandatory `disposition`** (arrest ≠ guilt), **sealed/expunged suppression** behind `person.legal-record.read-suppressed`, and a `geo_countries` jurisdiction FK; Ban-the-Box/FCRA display rules are an open seam (D-LegalRecords) | M5 |
...
| **M51** | Service identities | Machine clients (facades, connectors) authenticate via the external IdP's **OAuth2 client-credentials** flow through the existing OIDC/JWKS middleware, resolving to **service principals** — a registered machine subject kind carrying **per-principal grants** `(permission_code, org_id NULL)`, never role assignments and never unit reach (D-ServiceIdentities *amended*: instance-scope codes are admin-only via `pdp.go`, so a role could not carry `import.manage`); admin-managed `(issuer, subject)` registry generalizes the M16 `hermenea-importer`; env-secret path kept as minimal-install fallback. RLS service arm + org-owned operational writes deferred to M55 (split out of D-ConnectorPlane) | M8, M7 |
| **M52** | Console facade (headless topology) | **console-bff** — the first facade: owns the browser session, forwards the **end-user's token** (no on-behalf-of), speaks the Conjure API via the M27 SDKs. Realized as the **console's own Next.js server tier** (which already met every constraint) rather than a new deployable — D-HeadlessTopology *amended*; oikumenea leaves the public network in the compose topology (`8443` **and** `8444` unpublished). Also lands the **service-principal admin console** M51 deferred here, and the console's first permission gating (`can()`) | M27 (M51 optional) |
...
| **M59** | The cross-module chart & the first gated facets | The two things M57/M58 recorded and did not build: the `org` facet on `link__member_of` + `ChartDef.source`, which together draw the headcount-by-unit chart M57 left unbuilt; and D-ObjectFacets **rule 2's first live cases** (`vehicle.registrationCountry`, `account.holderKind`), which force the rule's missing LIST side — a gated facet's filter arg is refused, not omitted | M58 |
...
| **M22** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | verified — recursive `religion_taxa` tree + closure (D-Religion **refined** 2026-06-19: level-marker catalog, theism classification w/ nearest-declared-wins + unit override, M:N org classifications/one primary, Wikidata anchor). Migration `0023_religion` + curated 98-taxon seed (deep Christianity incl. major historic churches + world religions) via `deploy/religion-presets`. `internal/religion` module (raw-pgx repo, audited app svc reusing tenant for canonical-graph `createChildOrg`, transport on `religion.read`/`religion.catalog.manage`/`religionorg.manage`) wired into `main.go`; `pkg/rid` service 16. **Integration tests green** (seed+inheritance+override, reparent cycle guard, profile+one-primary, effective-type taxon+unit override, `excludes_child_creation` blocks child + canonical edge). `/religion` web taxonomy browser + create + theism-tag editor (type-checks + `next build`). **Live HTTP click-through (2026-07-22, dev stack + real Keycloak admin token):** boot clean; read taxon-ranks / 12 theism classifications / the 100-taxon tree (christianity…); christianity → `monotheistic` (declared, with `eng`/`ukr` i18n map); a created child denomination **inherits** `monotheistic` (nearest-declared-wins), a PUT-classifications **override** flips its effective to `nontheistic`; the reparent **cycle guard** prevents the cycle (christianity stays a root). The click-through **caught a real bug — the cycle guard returned 500** (the 3 religion `FAILED_PRECONDITION` errors — cycle / still-referenced / excludes-child — mis-served as server errors); **fixed → `CONFLICT` (409)**, matching tenant's `UnitCycleDetected`, re-verified live at **409** (org-profile classification/one-primary stays integration-covered). Org-classification live-test not re-run (integration-green). |
...
| **M37** | ✅ | ✅ | ✅ | ✅ | ➖ | ✅ | verified — first-party login security log ([D-LoginSecurityLog](architecture/roadmap-decisions.md#d-loginsecuritylog--a-first-party-loginip-security-log-on-the-federation-seam-extends-l-authzonly) + its 2026-07-21 as-built note). `account_login_events` (account Object `9,1,4`, migration `0043`), `pii:contact`. **Emit:** the OIDC/JWKS middleware records a login/IP occurrence per validated **human** request (`registration` on the JIT link, else `login`), **off the critical path** (detached goroutine) + best-effort (never fails the request); service principals excluded. **Bounded, not a firehose:** deduped to one row per `(account, context, ip)` per `login-security.dedup-window-seconds` (default 1h) — a bump of `last_seen_at`/`occurrence_count`, else insert. **Client IP through the facade:** console-bff forwards a single authoritative `X-Forwarded-For`, trusted only when `login-security.trust-forwarded-for` is on (narrow **D-HeadlessTopology amendment**); else `RemoteAddr`. **IP-intel** (`resolved_country`/`_isp`/`is_vpn`/`is_tor`) is a deferred `IPIntel` no-op seam (NULL; a future hermenea connector). **Read** `GET /accounts/{id}/login-events` on the instance-scope `account.security-log.read` (admin-only). **Erasure/retention:** `SubscribePersonPurge` hard-erases in the purge tx (fan-out width 18→**19**); `delete_login_events_before` + `login-security.retention-days` (0 = forever, best-effort boot sweep). Verified: unit (`clientIP` XFF-on/off + host/port + IPv6 stripping) + integration on `oikumenea_test` (dedup within window / new row past window / distinct `(context,ip)` / no-op-intel NULLs / keyset paging / `DeleteBefore` sweep / **`EraseByPerson` erases the purged person's events only**) + the fan-out width-guard baseline bump + `go build`/`vet`/full unit sweep/`atlas validate`/`tsc`/docs-links green. UI `➖`. **Live HTTP e2e** (token→emit→row over the wire, and the 401/403/200 read gate) deferred to the **docker stack** — the host-binary boot faces the same throwaway-DB `gen_random_bytes`/`search_path` blocker seen with M22; the transport gate reuses the M51-proven `RequireAnywhere`(instance-scope) path |
| **M38** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | verified — criminal/arrest/court records ([D-LegalRecords](architecture/roadmap-decisions.md#d-legalrecords--category-level-criminal--arrest--court-records-piispecial-gdpr-art-10-extends-d-specialpii-d-overlayfoundation-d-geo)). Migration `0016_legalrecords`: `person_legal_records` (object `6,1,22`, `pii:special` GDPR **Art. 10** — the category-level offence `detail` is **envelope-encrypted** + blind-indexed like M36 health; plaintext `kind∈{criminal_conviction,arrest,court_judgment}` + a **mandatory `disposition`** (arrest ≠ guilt); **many rows per person**; jurisdiction hard-FK → `geo_countries`; NOT-NULL `legal_basis`; never inferred; crypto-erased on purge). **Suppression:** `is_suppressed`/`suppressed_reason∈{sealed,expunged}` mark a **retained** record withheld from `person.legal-record.read` and revealed only to a holder of the stricter `person.legal-record.read-suppressed` (in **no** base role) or an instance admin — computed in transport via `AllowedAnywhere` and passed as `includeSuppressed` (the R-31 redaction pattern). New `person.legal-record.read` code folded into `sensitive-reader`; `read-suppressed` in no base role. `PersonService` list/upsert/delete (`personsensitive`: `application/legal.go`, sqlc + adapter with jurisdiction code↔RID resolution, `ErasePerson` extended; transport `legal.go`); action registry (`person.legal_record.upsert/delete`) + endpoints/params regen (fixed a pre-existing generator drift: M53/M54 `connector.*`/`import.locales` synced into the `exempt` map) + `pkg/rid` + `ontology-mapping.md` §3 + `rewrap` table list + `datascope_gating_test`. Go+TS SDK regen; `/persons/[id]` **Criminal & court records** card (kind/disposition/jurisdiction/suppression, art6 legal-basis picker) + 4-locale glossary. Unit + integration green on `oikumenea_test` (envelope-at-rest holds no plaintext + blind index + decrypt round-trip; disposition/kind/legalBasis/jurisdiction-FK validation; suppression filter hides sealed unless includeSuppressed; purge crypto-erases to tombstones); `mapError` drift-guard extended; web `tsc` clean; **live HTTP e2e** on the host binary vs the dev DB + real Keycloak (create 200 w/ jurisdiction resolved, missing disposition → 400, sealed upsert 200, admin list shows suppressed, delete 204, raw ciphertext is binary/holds no plaintext) |
...
| **M51** | ✅ | ✅ | ✅ | ✅ | ➖ | ✅ | verified — service identities ([D-ServiceIdentities](architecture/roadmap-decisions.md#d-serviceidentities--machine-clients-authenticate-via-idp-client-credentials-and-resolve-to-service-principals-extends-d-hermenea-l-authzonly), **amended**; [north star](architecture/north-star.md) plane 2; designed in [identity-federation.md](modules/identity-federation.md#service-principals-m51)). Machine callers authenticate via the IdP's **client-credentials** grant through the SAME OIDC/JWKS middleware and resolve to a **service principal**. **Amendment:** principals hold flat **per-principal grants** `(permission_code, org_id NULL)`, never role assignments — `authz_role_assignments.subject_person_id` is a hard person FK and `pdp.go` satisfies instance-scope codes (`import.manage`, the M53 wiring codes) only for instance admins, so a principal given a role could not import at all. `org_id` NULL = instance-wide; a named org confines a connector (the org, not the unit, is a scraper's blast-radius boundary). Migration `0039_service_identities`: `account_service_principals` (`9,1,3`) + `authz_principal_grants` (`8,2,3`) + `audit_log.actor_principal_id` (no third `actor_type` — D-Audit holds) + **symmetric triggers** keeping an `(issuer, subject)` a person identity XOR a principal. Middleware resolves person **first** (human hot path unchanged, M47's zero-query-warm preserved) then the machine arm; a rejected unknown token logs `issuer`/`azp`/`subject` for copy-paste registration. A service request gets **no authority snapshot and no RLS pinning**, so every person-shaped PEP path denies it structurally — made explicit in `SubjectAuthority`/`FilterVisibleUnits`. New `RequireService`/`RequireServiceOrPerson`; `RequireImport`'s hard-coded `hermenea-importer` exemption **deleted** in favour of a real revocable grant. The `HERMENEA_OIKUMENEA_TOKEN` fallback is boot-seeded as a real principal on reserved issuer `urn:oikumenea:local` (boot guard refuses that issuer in `idp.issuers`). New codes `service-principal.read`/`.manage` (instance-scope, in no base role). Verified: unit (PEP machine-denial matrix incl. the unbound-enforcer proof that no principal path reaches the PDP, org-scoping table, catalog/base-role guards, audit actor shape, mapError drift guard) + integration on `oikumenea_test` (registry resolution, **collision rejected in both directions**, disable/re-enable, immutable identity key, idempotent `EnsurePrincipal`, grant validation, org scoping, both partial uniques, immediate revoke, `authz_epoch` bumps) + **full 49-package integration sweep green** + **live e2e on a fresh DB with real Keycloak client-credentials**: unregistered → **401** (with the telemetry), register+grant → same token **200**, audit names `e2e-connector` (autoseeder imports correctly carry none), shared-secret still imports and now names the seeded principal, machine on `/person/v1/persons` and `/tenant/v1/units` → **403** (RLS arm is M55), revoke → **403 immediately** (grants uncached), disable → **401**. Boot assertion caught a real gap (the new link type needed a D-LinkTraversal exemption). UI `➖` — the principal console **landed with M52**, which also closed two M51 leftovers its drift guards had flagged: `authz_principal_grants` (`8,2,3`) was unclassified in the D-Temporal tier map (now `tierValidity`, matching its `granted_at`/`revoked_at` siblings) |
| **M52** | ✅ | ✅ | ✅ | ➖ | ✅ | ✅ | verified — console facade / headless topology ([D-HeadlessTopology](architecture/roadmap-decisions.md#d-headlesstopology--oikumenea-is-internal-only-behind-unprivileged-user-token-passthrough-facades-extends-l-authzonly-amends-d-webui), **amended**; [north star](architecture/north-star.md) plane 3). **Amendment — no new deployable:** the facade was already built. The console's Next.js server tier (`web/src/auth.ts`: Auth.js v5, OIDC code exchanged server-side into an httpOnly session; `web/src/app/api/oikumenea/[...path]`: the bearer-attaching passthrough proxy) already does session + passthrough + hosting under exactly this decision's constraints, so **the console's server tier IS `console-bff`**; a separate Go BFF would have had to duplicate or replace Auth.js's session for no security gain. The compose service is renamed `web` → `console-bff` to say so. **The core needed zero changes** — no trusted-proxy list, no `X-Forwarded-*` handling, `azp` diagnostic-only — the facade is just another authenticated caller presenting a user's token. **Topology:** `app` publishes **no** host port; `8444` is unpublished too, deliberately (`/status` + `/debug` bypass auth via `isBypassPath`, so publishing it would leave an unauthenticated host-facing surface). `API_BASE_URL` loses its `https://localhost:8443` default and now throws at startup if unset (routes stay dynamic, so the Docker build is unaffected). **UI:** the service-principal console promised by M51 — principals table (register / edit display-fields-only / disable↔enable; no delete, disable is the kill switch) + per-principal flat grants `(permission, orgId?)` as child rows under `?principalId=`, issuer offered as a select over `listIssuers()`. First **permission-gating** in the console: `web/src/lib/api/can.ts`, a deliberate **best-effort deny** — `Whoami` carries no permissions so the question goes to the PDP, but `POST /authorize` is itself gated on `assignment.read` with no self-exemption (OQ-5), so a denied *question* collapses to `false`, which is the right answer anyway (**superseded**: `can()` without a `unitId` now answers from `GET /me/capabilities` — [D-SelfCapabilities](architecture/decisions.md#d-selfcapabilities--self-only-capability-introspection-get-mecapabilities-resolves-oq-5) — so gating works for non-admins too; the `POST /authorize` path survives only for unit-scoped questions); layout resolves codes server-side and passes them to the client `Nav`. Verified **live** against real Keycloak: scripted Authorization-Code login → session; click-through of 9 surfaces; full principal round-trip through the **BFF proxy** (register → grant → list → duplicate-grant **409** → unknown-code **400** → revoke → disable → enable); **revoke propagation** — instance-admin revoked through the facade, and with no re-login the nav entry vanished, the page flipped to the gated state, and the API returned **403**, all within the 2 s D-AuthzGrantCache bound; facade env confirmed to carry **no** service token. Migrated `➖` (topology + UI, no schema) |
...
| **M56** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | verified — list filters & the facet vocabulary ([D-ObjectFacets](architecture/decisions.md#d-objectfacets--one-per-object-type-facet-vocabulary-driving-both-list-filters-and-per-module-stats-endpoints-extends-d-visibilityscope-d-personreadscope-constrained-by-d-datascope) + [D-ConsoleDashboards](architecture/decisions.md#d-consoledashboards--every-listable-type-gets-a-list-view-and-a-dashboard-view-over-one-url-borne-filter-set-amends-d-webui); catalog in [facets.md](architecture/facets.md)). **Decided/Designed:** one `Facet` declaration per object type, consumed twice — typed Conjure query args on the list endpoint and a `groupBy` key on M57's stats endpoint. **Ticket 1 of 4 done — `pkg/listing`:** the shared cursor codec (single-column + tuple) and `PageSize` clamp, with 3 AST drift guards; 13 modules migrated, retiring 14 cursor-helper pairs and 15 clamp copies. **Fixed en route:** six transports (company/education/externalorg/finance/religion/vehicle) emitted base64 **StdEncoding** page tokens, whose `+` decodes to a space in a query parameter — now RawURL, with a tolerant decoder so pre-upgrade tokens still page. **Ticket 2 of 4 done — the facet kernel, on `person` (7 facets) and `unit` (retro-declared 4 + 3 new):** `pkg/facet` holds the vocabulary (`Facet{key,kind,table,column,readPermission,buckets}` + per-object-type registration + a boot `MustBeBound`, joined into the composition root's seam loop); `tools/genfacetargs` mirrors each list endpoint's query args **from the Conjure IR**, and `pkg/facet/args_test.go` fails the build in BOTH directions — a facet whose arg the contract does not ship, and an arg bound to neither a facet nor a *classified* non-facet role (paging/search/traversal, each class checked, so it cannot decay into an allowlist). `plaintext_test.go` enforces D-ObjectFacets rules 1–2 against the migration DDL with no database (a facet may name only a plaintext column; above `pii:basic` it must inherit its field's read code), including a contrapositive sweep over **every** `pii:special` column and a live negative fixture. One `PersonFilter` drives BOTH list paths — the instance-admin queries and membership's three read-scope visibility queries — with the identical predicate block, since a Go-side re-filter would return a short page with a `nextPageToken` (R-06); a no-DB SQL narg-parity test proves the block is present in all five queries, and a DB differential asserts `scoped(f) == admin(f) ∩ reach` over randomized filter sets. **Fixed en route (two):** the scoped list returned soft-deleted person ids that hydration silently dropped — a page shorter than `pageSize` **with** a next-page token; and the `unitId` facet answered **200 with zero rows** to an instance admin, because person's list read from the bare pool while the facet's predicate probes RLS-protected `membership_memberships` — the `app.*` GUCs were unset, so `authz_unit_in_reach` matched nothing. Only the **live** run caught the second (the integration suite connects as a superuser and bypasses RLS); the guard is a source check, at the layer where the mistake is made. **Measured** on the M46 scale world (`seed-scale -enrich`, new): the correlated subtree probe (242 ms) became an uncorrelated closure set (33 ms); `…Dense` now drives from `person_persons` (`unitId` 816 ms → 30 ms); no path seq-scans `person_persons`, asserted by an env-gated plan guard. Numbers in [review-2026-07](architecture/review-2026-07.md#m56-ticket-2--person-facet-filters-2026-07-28). `Migrated ➖` **confirmed by measurement**, not assumed: every filtered path is index-backed and the residual cost is scan width, which no index changes. **Ticket 3 of 4 done — the missing top-level lists:** `GET /memberships` / `GET /orders` / `GET /documents`, each with its facet args, its catalog block (`link__member_of` is the FIRST faceted reified link — `pkg/facet` widened to accept link tokens, and `rid.TokenOf` moved into `pkg/rid` so the facet catalog and the console registry cannot name a type differently), and both list arms sharing one filter block. **No implicit status default**, unlike the per-unit/per-person lists: a hidden active-only filter would make M57's `totalCount` disagree with its own status distribution, and would leave ended rows unreachable. Documents scope THROUGH THE HOLDER (no unit column, no RLS) — the semi-join is folded into SQL, so an unreadable holder's rows are absent rather than post-filtered (R-06). **Measured, and the measurement changed the design:** a materialized reach set collapses at root reach (documents 6 419 ms — the planner drives from the reach side and top-N sorts, so the LIMIT never terminates early), while a per-row point probe collapses at leaf reach (13 100 ms). Both shapes now ship per scoped endpoint, dispatched on reach cardinality exactly as `VisiblePersonIDsForSubject*` does — root-reach costs fell to 179–210 ms. **`Migrated` ✅:** migration `0017_facet_lists` adds three reach functions (set / point-probe / capped-count — one definition each, so the PDP parity contract is not copied again) and the status-agnostic keyset indexes the status-filterable list needs, since every pre-existing membership index is partial on `status='active'`. Guards: the narg-parity test now covers all five registered types and fails on a registered type with no parity group; a consolidated `db` source guard proves all six new read paths use the request-pinned connection (the ticket-2 empty-page bug shape); the differential test gained an `authz_readable_units` vs inline-CTE parity case over 25 randomized worlds. Numbers in [review-2026-07](architecture/review-2026-07.md#m56-ticket-3--top-level-list-endpoints-2026-07-29). **Ticket 4 of 4 done — the console, and the whole request state in the URL:** `ObjectTypeDef` gains `filters?: FilterDef[]` (28 defs across the five types) whose `params` carry the CONTRACT arg names verbatim rather than re-deriving `Facet.Args()` in TypeScript — the derivation has two live exceptions (`unit.level` pins the pre-existing scalar arg, `membership.effectiveFrom` pins `effectiveFromAfter`/`Before`), so a second copy of the rule would be a second thing that can drift; **render arity therefore comes from `params`, never from `kind`**, which is the one mistake that would slip past every guard. One URL kernel (`web/src/lib/ontology/filters.ts`) now feeds the outbound query, the pager, the view toggle and the chips: `exploreHref` rebuilds the whole `URLSearchParams` and **always drops `pageToken`**, so "a filter change returns to page 1" is a property of the builder rather than discipline at five call sites, and `readFilters` whitelists by `def.filters`, so a junk param survives in the URL (link stability) but never reaches the API. Retired: the four hand-concatenated query strings, and the two `<form method="get">` that submitted only their own fields and so silently wiped each other. **Two bugs fixed en route:** the `?view=tree` toggle dropped every filter but `org` (its href was hand-built), and `NewUnitForm` / `UnitSelect` passed `pageSize` as `listUnits`' 8th positional arg — which ticket 2's facet args had turned into `graph`, latent only because the TS SDK was still stale and shipped no `listMemberships`/`listOrders`/`listDocuments` at all. **The first `kind:"link"` type in `EXPLORABLE_TYPES`** (`link__member_of`) exposed a naming collision — three tool pages shared their labels with the new explore entries — resolved by renaming the *tools* for what they actually are (Membership lookup / Order desk / Document catalogs), not the lists. **Guard (the third):** `pkg/facet/console_test.go` parses `registry.ts` and holds `FilterDef[]` against the catalog in both directions plus the IR mirror, with a `consoleExempt`-with-reason map, a non-vacuity floor (a broken regex parse goes red instead of vacuously green) and a live-negative fixture; hand-broken three ways — dropped facet, re-derived `levelMin`/`levelMax`, renamed `filters:` key — each confirmed red. **Honesty fixes, both recorded as seams in [facets.md](architecture/facets.md):** the page-local quick filter survives only where the endpoint ships no search arg and now says "on this page" (the old "N of M" beside a keyset table read as a total), and the column sort stays client-side because no endpoint ships `orderBy`. **Verified live** (`oik serve` + `next dev` on the seed-demo DB): filters honoured and shareable, chips and 4-locale labels, Table↔Tree preserving the filter set, `status=ended` reachable ONLY through the new top-level list, `unitKind` disabled until a domain is picked, and 12 URL-kernel invariants asserted directly (the request log carries no query string, so the whitelist is proven at the module level, not on the wire)
| **M57** | ✅ | ✅ | ✅ | ➖ | ✅ | ✅ | verified — module dashboards ([D-ObjectFacets](architecture/decisions.md#d-objectfacets--one-per-object-type-facet-vocabulary-driving-both-list-filters-and-per-module-stats-endpoints-extends-d-visibilityscope-d-personreadscope-constrained-by-d-datascope) + [D-ConsoleDashboards](architecture/decisions.md#d-consoledashboards--every-listable-type-gets-a-list-view-and-a-dashboard-view-over-one-url-borne-filter-set-amends-d-webui); catalog in [facets.md](architecture/facets.md)). **Ticket 1 of 4 done — the stats kernel and the first two endpoints:** `GET /person/v1/stats/persons` + `GET /tenant/v1/stats/units`, each taking EXACTLY its list endpoint's filter args plus a `facets` CSV and returning `totalCount` + per-facet buckets. New stdlib-only `pkg/stats` owns everything that is not SQL: the CSV selection (absent = every facet the caller may read; `facets=` = the total alone; an undeclared key is a 400, a facet whose read code the caller lacks is silently OMITTED — rule 2), and bucket assembly (enum values zero-filled in CHART order, ages assigned to the catalog's bands, top-N tails collapsed, NULLs surfaced as the mandatory `(unknown)` bucket) under one invariant: **every counted row lands in exactly one bucket**. **Three specified things changed, each for a reason worth reading:** (a) the path is `/stats/<collection>`, because `/persons/stats` beside `/persons/{personId}` makes httprouter **panic at startup** — `go build` and every unit test stay green, so a contract-wide route-conflict guard now fails the build on that shape (keyed by METHOD: one httprouter tree per method, which is why the pre-existing `DELETE /rank-scheme/{level}` beside `POST /rank-scheme/systems` is legal); (b) one static aggregate query per (module, ARM) rather than per (module, facet) — a materialized candidate CTE carrying the list's filter block verbatim, then one `UNION ALL` branch per facet gated on a `want_*` flag the planner uses to SKIP it, so a whole dashboard is one scan and an unreadable facet is never grouped; (c) a scoped aggregate needs only ONE plan shape. **(c) was measured, not assumed:** with no `LIMIT` to terminate early, the materialized reach set beats the point probe at EVERY reach — 8.3 / 79.8 / 7 144 ms at leaf / mid / root against 12 926 / 17 066 / 24 869 ms — the exact inverse of the ticket-3 list verdict, so the sparse/dense dispatch and its ~180 ms tax are deliberately absent. **Exit criterion proven:** `stats.totalCount` equals the row count from exhaustively paging the same list under the same filters, asserted per filter on BOTH arms (14 person filters × admin/scoped, 8 unit filters), plus the sum invariant per facet, the omission rule, and — the property the separate scoped queries exist for — that an unreadable person and an out-of-reach shadow unit move neither the total nor any bucket. **Scale (M46 world, 10^6 persons):** mid-reach subject draws all seven facets in 873 ms, leaf in 245 ms, admin whole-directory in 6.0 s; a root-reach subject costs 14.5 s, recorded as an honest residual with the `facets` CSV as the lever and estimation still refused (it would break the exactness the differential test asserts). `Migrated ➖`: no schema — the aggregates reuse migration 0017's reach functions. Guards: aggregate halves held **byte-identical** across arms (hand-broken, confirmed red), every branch proven to name a declared facet AND read its want flag, narg parity extended to all six new queries, the RLS request-pinned-connection source guard extended to the stats paths (an unpinned aggregate returns a confident ZERO rather than a short page), and a boot assertion that every ref facet's target type has a label resolver. **Live-checked** (host binary against the seed-demo dev DB + a Keycloak token), because the integration suite connects as a superuser and cannot see an RLS mis-wire: both endpoints answer, the server BOOTS at all (which is the route-conflict proof), ref buckets carry resolved names (`Ukraine`, `1/1/1 Platoon`), the rank distribution comes back in SENIORITY order rather than by count, every facet sums to `totalCount` on real data, `facets=` returns the count alone and an undeclared facet key is a 400. Numbers in [review-2026-07](architecture/review-2026-07.md#m57-ticket-1--the-dashboard-aggregates-2026-07-29). **Ticket 2 of 4 done — the remaining three endpoints, and the M57 tranche is now BACKEND-COMPLETE:** `GET /membership/v1/stats/memberships` + `/order/v1/stats/orders` + `/document/v1/stats/documents`, each a repetition of ticket 1's shape (one materialized candidate CTE carrying the list's filter block verbatim, one `UNION ALL` branch per facet gated on a `want_*` flag the planner skips) — so `statsPending` in the arg-drift guard is now EMPTY, which is the guard's way of saying every registered type ships a dashboard. **The verdict was re-measured per table rather than inherited:** ticket 1 proved the reach SET form beats the point probe for aggregates on `person`, whose reach lands on the membership row, but `document` reaches THROUGH THE HOLDER and is the case that broke the set form for lists — measured, the set form still wins (documents 25.7 / 218 / 4 322 ms at leaf / mid / root against the probe's 12 447 / 15 771 / 23 651 ms), so all five types ship one scoped query and none pays the lists' ~180 ms dispatch tax. **Two bugs the integration suite structurally could not see, both found by the live run** (it connects as a superuser and bypasses RLS): the label resolvers read the BARE pool, so any row-secured label table silently resolved nothing — `positionId` buckets came back unlabelled the moment a labeler was pointed at RLS-protected `membership_positions`, and `tenant_units`' public-read arm had been hiding the same hole for in-reach SHADOW units in D-LinkTraversal labels (now `db.RequestQuerier`); and the ARM CONVENTION was one edit from a leak — an empty subject means "no visibility predicate" and `pep.SubjectAuthority` returns `("", false)` for a machine subject, so a non-admin with no subject would have received whole-instance counts. That rule now lives once in `stats.Compute`, which all five types route through, asserted per module. **Honest cost, recorded not smoothed:** `link__member_of` all-facets is 9.6 s admin / 11.1 s root, and 8.6 s of it is ONE facet — `personId`'s top-N ranks 10^6 distinct persons to keep fifteen. It is a FILTER facet, not one of the catalog's charts; the dashboard **as facets.md draws it** (status tiles + intake curve + billet-fill) costs **1 342 ms admin / 3 229 ms root / 29.7 ms mid**, which is exactly what the `facets` CSV is for, and the bounded-top-N fix is costed and deliberately unbuilt. `MATERIALIZED` was checked too: inlining measures the same (9 562 vs 9 374 ms), so the cost is the aggregation, not the spool. **Also fixed:** a `dateTrunc` facet's `(unknown)` bucket now survives being empty — an order register with no drafts must still show a zero draft backlog, and ticket 1 could not have caught it because `person`/`unit` declare no `dateTrunc` facet. `Migrated ➖`: no schema — the aggregates reuse migration 0017's reach functions. Guards extended in all four places (aggregate halves byte-identical across arms for the three new types, narg parity over the six new queries, the RLS pinned-connection source guard, and the two-directional stats-arg mirror). Numbers in [review-2026-07](architecture/review-2026-07.md#m57-ticket-2--the-membership--order--document-dashboards-2026-07-30). **Ticket 3 of 4 done — the charts, and no client JS at all:** seven primitives in `web/src/components/charts/` over `@visx/scale` + `@visx/shape` alone (`BarChart`/`DonutChart`/`Histogram`/`PyramidChart`/`StatTile`/`Sparkline` + the card frame), `dashboard?: DashboardDef` on `ObjectTypeDef` (23 `ChartDef`s across the five types), and `?view=dashboard` generalizing the unit-only Table/Tree switch into a `ViewToggle` built on `exploreHref` — **the dashboard is the DEFAULT view** of any type that declares one (opening a collection answers "what is in here" before "what is on page 1"; the aggregate describes the whole filtered set, a keyset page describes fifty rows), with `?view=table` the opt-out and the default view the one that clears the param, so a collection's canonical URL stays paramless (amends D-ConsoleDashboards) — so "the filter set survives the toggle" and "a view change returns to page 1" stay properties of the URL builder rather than discipline at a new call site. **Every chart is a Server Component:** a segment is an `<a>` to the same URL with one more filter applied, the readout is a native `<title>`, and every mark carries its value as a direct label — which is also what discharges the palette's contrast relief rule. Confirmed, not assumed: **no client chunk contains visx or d3**. **The new kernel is the INVERSE map** (`lib/ontology/buckets.ts`): a bucket key back to a filter patch, in one place, with everything that CANNOT be expressed returning null rather than an approximate link — `(unknown)` (no arg says "column IS NULL", and every date bound excludes nulls), `(other)` (a top-N tail is not a value), and a band whose facet ships only a scalar arg. **That last case was then CLOSED in the same session** (`levelMin`/`levelMax` on `listUnits` + `unitStats`): M56 deferred the range args "to when the bands are actually consumed", and drawing a band chart is what consumes them, so `unit.level` now binds the derived pair and its bars link like every other segment. The pre-existing scalar `level` is retained and still honoured — the contract is expand-only and all three predicates are ANDed — and is classified under a new, fourth non-facet class `superseded`, whose check is earned rather than written down: the successor named must exist, be a RANGE facet over the SAME column, take the same Conjure type, and not itself bind the arg. `TestUnitStatsLevelBandsAreClickThrough` holds every band's bucket against the row count that band's own bar-link returns (hand-broken `<=`→`<`, confirmed red on four bands), and an inverted `levelMin > levelMax` is a 400 rather than a silently empty page. **Fixed en route, the third appearance of one bug shape:** `UnitSelect`/`NewUnitForm` called the POSITIONAL `listUnits(org, …, 200)`, so two args inserted in the middle moved `200` from `pageSize` onto `rootsOnly` — both now use the generic `api.request("GET", "/tenant/v1/units", { query })`, which cannot shift when the contract widens. **Two charts cost a second request, and that is the design rather than a workaround:** the age pyramid is a cross-tab a per-facet endpoint cannot answer, so each wing is the same request state plus `sex=<value>` and `facets=birthdate` (and it collapses to a single-series histogram when `sex` is already filtered); the document "expiring within 90 days" tile is a bounded count (`facets=` — the total alone), because the window does not fall on month boundaries and summing month buckets would be off by up to two. Each dashboard otherwise asks for EXACTLY the facets it draws. **Not built, and recorded as a contract gap rather than an omission:** facets.md's `unit` ⑤ headcount-by-unit — the unit dashboard is org-scoped but `membershipStats` ships no `org` arg and `membership.unitId` is exact-match, so the chart would have mixed organizations. **Guard (the fourth consumer):** `pkg/facet/dashboard_test.go` holds every `ChartDef` against the catalog — an undeclared facet key is a **400 on the whole request**, so one stale chart blanks the dashboard rather than itself — plus the dashboard `path` against the **Conjure YAML** (a hand-typed `/documents/stats` is the shape httprouter panics on), tone keys against their enum's CHECK set, a pyramid's `splitBy` against the real facet args, and the console's `buckets:` declaration against the catalog's bucket strategy (the click-through inverts a bucket key, and the inverse of an age band is not the inverse of a calendar month); non-vacuity floor plus live-negative fixtures, hand-broken four ways, each confirmed red. **Ticket 4 of 4 done — verified live** (`oik serve` + `next start` on the `scripts/seed-demo` DB, Keycloak session): all five dashboards render (42–169 ms; person is the only multi-request one), and the console's own claim — that CLICKING a bucket reproduces the filters that counted it — was checked **exhaustively rather than sampled**: all 14 age-band wings and all 9 populated expiry months plus the 90-day tile matched exhaustive paging of the list each segment links to, **exactly**, including the open-ended `65+` and the `+1 day` boundary. Also confirmed live: RLS-protected `membership_positions` labels resolve all the way into a chart segment (M57 ticket 2's bare-pool bug stays fixed), `sex=female` → Table keeps the filter and drops the view, and the 4-locale switch re-renders titles and wing labels through the same enum labels the filter `<select>` uses. Three seed-data observations recorded in [review-2026-07](architecture/review-2026-07.md#m57-tickets-3--4--the-charts-and-the-live-click-through-2026-07-30) (no document expiry dates, a draft order carrying an issue date, NULL unit levels) — data, not defects. Depends on M56 |
| **M58** | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | verified (all 7 tickets) — dashboard & filter coverage rollout across the remaining listable object types ([facets.md](architecture/facets.md) catalog, M58 tranche) + ontology-registry entries for the six modules that had none (religion + external-org in ticket 2, vehicle + finance in ticket 3, **company + education in ticket 5 — all six done**). **Ticket 1 of N done and verified — `audit`, and it was not the trivial repetition the plan assumed.** Its nine filter args did need no contract churn, but every OTHER assumption the M56/M57 kernel had baked in turned out to be about the five operational-core types rather than about facets, and doing audit first surfaced all four at once. **(a) A collection with no RID type token:** an audit row records an Action whose RID belongs to the PRODUCING service, so there is no `audit` triple to register and inventing one would make `rid.TokenOf` describe identifiers that never exist. New `ObjectType.Ledger` — a one-field escape carrying its own reason, refused for a token that IS registered, and held to AT MOST ONE type by a guard, so a second ledger must be argued rather than copied; the kind rule itself stands (an action *invocation* is still not a collection, but the RECORD of one is a row like any other). **(b) An open value set:** `action` has a registry (pkg/action, 288 codes) but no CHECK, and `target_type` has neither — so a fifth kind, `KindCode`, ranked top-N with **the key as its own label** and forbidden from naming a RefType. **(c) Visibility that is not a predicate:** `audit_log` is FORCE RLS with a `unit_id` probe and the gate is the coarse `audit.read`-anywhere, so the aggregate ships **ONE arm** where every M57 type ships two — safe for exactly one reason, that it reads the request-pinned connection, so the `db` source guard was extended to cover it (and parameterized over the module's accessor, audit's being `reader`/`RequestDBTX`). **(d) A day-grain axis, the first:** an audit trail is read day by day and a monthly bar hides the spike it is read for, so the console gained `daySpan` + `fmtDay`; `since`/`until` are Conjure DATETIMEs, so the console declares `argType: "datetime"` and ONE helper widens a picked day to its RFC-3339 endpoints for both the filter control and the histogram's click-through. **The ledger is month-partitioned and unbounded**, so the dashboard link carries a **30-day window in the URL** — a visible, clearable chip, never a server-side default that would make `totalCount` disagree with the caller's own filters. `/audit`'s bespoke page (positional SDK args, two of nine filters, a dropped `nextPageToken`) is now a redirect to `/explore/audit`. **Two bugs found by the LIVE run and by nothing else:** `Selection.TopN()` looked only for a REF facet, so a selection of code facets alone bound `top_n = 0` and **collapsed every bucket into `(other)`** — not empty, not an error, entirely wrong; and `facets=` is NOT "count only" (an empty list means every facet; a list of nothing, `,`, means none), so M57's document expiring-soon tile had been fetching the whole dashboard a second time. **Guards (five, each hand-broken and confirmed red):** Ledger-earns-its-exemption, code-facets-carry-no-label-promise, console `argType` vs the IR mirror, an aggregate-group coverage floor that refuses a stats endpoint with no parity group, and the RLS pinned-connection guard extended to audit. **Verified live** (`oik serve` + `next start` on seed-demo): the dashboard renders in 44–73 ms, `/audit` 307s to the explorer, and every clicked segment — action, outcome, targetType, actorType and both day buckets — lands on exactly the rows it counted, checked against exhaustive paging (9/9, 27/27, 9/9, 27/27, 18/18, 9/9). Numbers and the four assumptions in [review-2026-07](architecture/review-2026-07.md#m58-ticket-1--the-audit-dashboard-and-what-a-ledger-broke-2026-07-31). **Ticket 2 of N done and verified — `external_organization` + `taxon`, the first two VERTICALS, and the taxonomy broke a fifth and sixth assumption.** **(e) A distribution that does not partition:** a tree cannot sum to `totalCount` — `subtree` counts each taxon under every ancestor and `classification` under every effective theism tag — so new `Facet.NonPartitioning` exempts **the summation assertion and nothing else**, leaving untouched the property the vocabulary actually rests on (a bucket's count equals the rows its own filter returns), held by two guards: a substantive reason, and the facet's table must not be the listed table (a row has one value in its own column, so it CANNOT overlap — an exemption there is imitation, not need). The exact-parent alternative partitions honestly and then **dead-ends** after one click; the closure facet drills all the way down. **(f) The kernel assumed a module's SQL is static text:** religion, externalorg, vehicle and finance build SQL at runtime and ship no `queries/*.sql`, so `pkg/facet/rawpgx_test.go` carries the same invariant by **AST** — both paths call ONE shared filter builder, plus a single named aggregate const — and the two sqlc-shaped coverage floors DEFER to it rather than exempting the types. Also: both `kind`/`rank` args took catalog CODES while a ref bucket key is a RID, so both were widened to accept either (`ClassSuperseded` is restricted to RANGE kinds). **Two real bugs, each found by exactly one method:** `listTaxa` had been **silently losing rows since M22** — ordered by `rk.ordinal,…` but paged on `t.id > cursor`, dropping **16 of 100** taxa from page 2 onward, caught by the M57 differential on its first run and fixed to a keyset on the unique indexed `code`; and every **month segment of a datetime facet linked to a 400**, because `monthPatch` never learned the RFC-3339 widening `dayPatch` has (no earlier month-grain facet takes datetimes) — the chart rendered perfectly and every bar was dead, found only by clicking one against the running server. **Guards (six, each hand-broken and confirmed red.)** **Verified live**: 45/45 taxon and 15/15 external-org buckets land on exactly the rows they counted against exhaustive paging; the subtree drill descends 100→55→22→3 with every step exact; 67/67 console-rendered segments navigate to a working filtered list; browsing moved out of both bespoke pages while their editors stayed. Numbers in [review-2026-07](architecture/review-2026-07.md#m58-ticket-2--the-first-verticals-and-what-a-tree-broke-2026-07-31). **Ticket 3 of N done and verified — `vehicle`, `account` and `card`, the two remaining RAW-PGX modules — and it is the first M58 ticket that took NO new kernel exemption:** no `Ledger`, no `NonPartitioning`, no new `Kind`, and all **14 distributions sum to `totalCount`**. It was chosen because `pkg/facet/rawpgx_test.go` had only ever been applied to the two modules it was WRITTEN ALONGSIDE, and a guard that has never described a type it did not grow from is a guess; it held unamended. **`registrationCountry` could have taken `NonPartitioning` and did not** — `vehicle_registrations` is one-to-many ownership history and is not the listed table, so the exemption was available and legitimately earned, but confining the facet to the ACTIVE registration (at most one per vehicle) partitions honestly AND answers the question the chart is read for; an exemption being available is not a reason to take it. **`card` had no collection to describe, so the ticket built one:** `GET /cards` is new and METADATA ONLY — the same projection the per-account list already returned under the same `finance.read`, so it widens the SCOPE of a permitted read and discloses no new field, which is why no new permission code was needed despite PCI-DSS CDE scope; the per-account list became `listAccountCards` and every HTTP path is unchanged. **The `vehicle.color` defect facets.md recorded DID NOT EXIST** — `color_id` has been a hard FK to `platform_colors` since M42, via an `ALTER` 600 lines below the `CREATE TABLE` the ticket-2 survey read in the same consolidated file; since the 46→15 consolidation **a table's shape is no longer its `CREATE TABLE`**, and the colour chart ships as originally catalogued (nine palette hexes painted into the bars, with a hairline outline on fills too light to have an edge). **Three real defects, none in the facet code:** seven faceted columns carried no pii classification (migration 0019 — the third module in a row with that gap); `seed-demo` never REFERENCED four catalogs it seeds, so three ref facets and the fleet-age histogram each had exactly one `(unknown)` bucket and **the click-through they exist for could not be exercised at all**; and the guard-breaking harness needs `-count=1`, because these guards read files outside their own package and Go's test cache does not invalidate. **Guards: 10/10 hand-broken and confirmed red. Verified live**: 107/107 buckets land on exactly the rows they counted (including all 54 month buckets and 9 colour buckets), 14/14 distributions sum to total, **284/284 console-rendered segments** navigate to a working filtered list, dashboards render in 38–117 ms, `GET /cards` carries no PAN while `getCard` does, and all four bespoke editors survive the browse split. Numbers in [review-2026-07](architecture/review-2026-07.md#m58-ticket-3--the-raw-pgx-tranche-and-a-guards-first-real-test-2026-08-01). **Ticket 4 of N done and verified — `organization` + `languoid`, the only two remaining types whose list endpoint already worked unconditionally** (every other candidate needs a list mode invented first, and none of them can be exercised on the current dev data — 0 role assignments, `type_id` NULL on all 40 locations). **The ticket's finding is that `gateUnits` gates organizations and, for an organization, gates nothing:** the org list is shadow-gated by the *unit* gate, whose non-admin probe matches only on `a.target_unit_id = cand.unit_id` — and `authz_role_assignments.target_unit_id` is `NOT NULL REFERENCES tenant_units`, so an organization RID can never appear in it and the reach set is **empty by construction**. The tranche table was **right about the shape and wrong about the predicate**: two arms yes, but the scoped one is `AND visibility = 'public'`, not unit's `public OR in-reach`. Copying unit's predicate would compile, pass every guard and count identical rows *today*, diverging only when org reachability is fixed — at which point the list would still show zero shadow orgs while the chart showed some. Asserted rather than assumed, since it is a claim about another module's schema: a test grants **subtree** read over the shadow org's own unit, checks the reach really took, and shows the count does not move; it is written to go **red** the day the gap closes. **That gap is now CLOSED in a follow-up** (amends **D-VisibilityScope**): of the three shapes, **deriving org reach from unit reach** was chosen — an organization is visible when any of its live units is in the subject's reach. The other two (a nullable `target_org_id`, an org-level `scope`) add a PDP primitive for a question unit reach already answers; derivation also **discloses nothing new**, since `listUnits` takes the org RID as a REQUIRED arg and gates the *units*, so a subject with reach inside an org could already enumerate them. It is precise — live, a subject reaching `privatbank` sees it and still 404s on `upc-parish`. `gateOrgs` is a **sibling** of `gateUnits`, not a call into it with a flag: the two ask different questions, and a shared entry point is exactly what let an organization RID reach the unit probe unnoticed for eighteen milestones. No migration (`tenant_units_org_idx` already matches the predicate); scoped dashboard 8→22 ms. **Three guards behaved as designed:** the pinned unreachability test went red carrying the message written for that day (replaced by `TestOrganizationReachIsDerivedFromUnitReach`, asserting **both** directions since either alone is satisfiable by a bug); `shadowgate_test.go` survived structurally unchanged because it was written structurally; and the application-layer test stayed green correctly, since it cannot see a transport gate. Live: admin 5/shadow 2/5 rows, insider 4/1/4, outsider 3/0/3, chart and list agreeing at every row. **Pulling on that finding turned up a LEAK, now fixed:** `getOrganization` was documented "(shadow-gated)" in the contract and applied **no gate at all** — the list trimmed shadow orgs for a non-admin while the point read on the same object handed them over to anyone holding `organization.read` (demonstrated live: list 3 rows without it, `GET /organizations/{RID}` **200** with its code, visibility and state). Fixed with `gateUnits`, the same helper the list uses, so the rule has one implementation and both surfaces move together when the reach gap closes; a gated-out org is `OrganizationNotFound`, **not** a 403, because `shadow` hides *existence* and a permission error would confirm the RID is real. **The first guard written for it was useless and hand-breaking is what showed that**: the obvious application-layer test (point read and list agree) stayed **green** when the leak was reintroduced, because the gate is applied per handler in the transport and an application test has no handler to lose it from. The guard that works inspects the handlers themselves (`transport/shadowgate_test.go`) and asserts each shadow-bearing read routes through `gateUnits` — structural rather than behavioural, for the same reason the arm predicate is, plus a companion check refusing any other handler that compares against `VisibilityShadow` on its own. **`macroarea` is the first COMPOSITE code facet** — set-valued, stored semicolon-joined (183 of 27 177 rows), grouped by the **literal string** because the filter is an exact match, so it partitions honestly; it could not have taken `NonPartitioning` anyway, since the kernel refuses that exemption when the facet's table IS the listed table. **`limit` → `pageSize` is the first WIRE BREAK the vocabulary has demanded**: `ClassPaging` covers only `pageSize`/`pageToken` and `listLanguages` was the lone holdout among twelve types, so the arg was renamed rather than the guard widened (contained — every consumer in-repo, hermenea does not call it, and the domain `Filter.Limit` keeps its name). **Two non-defects recorded so they are not re-litigated:** the pii sweep came back **clean for the first time in four tickets** (the obligation stands, only the answer changed), and `family_code`'s padding risk is **theoretical** — every glottocode is exactly 8 characters and the `::text` cast already strips, so the `rtrim` is belt-and-braces, measured rather than assumed (ticket 3 lost a round to exactly that kind of unmeasured claim). **One real defect, in the seeder:** every demo org carried the column defaults, so `visibility` and `state` would each have been a single bar — and the fix could not be applied, because the seeder short-circuits on existing demo data while `-reset` is blocked on any dev DB holding hand-made rows it does not own; the short-circuit now refreshes the idempotent spreads, so the seeder stopped being all-or-nothing. The same gap in the test fixture (every languoid there has a NULL macroarea) is closed by the integration test seeding its own spread. **Guards: 10/10 hand-broken and confirmed red**, one of them caught by eight guards at once. **Verified live** (`oik serve` + the standalone console, admin *and* a purpose-made non-admin): 5/5 and **27 177/27 177** totals vs exhaustive paging, **47/47** buckets landing on exactly the rows they counted, **7/7** distributions summing to total, **48 console segments** clickable with `(unknown)`/`(other)` correctly inert, dashboards in 9–44 ms, `?limit=5` now genuinely inert — and the headline differential: admin sees `totalCount` 5 / `shadow` 2 / 5 list rows, the non-admin sees 3 / 0 / 3, **chart and list agreeing on both arms**. Numbers in [review-2026-07](architecture/review-2026-07.md#m58-ticket-4--the-last-two-arm-type-and-a-gate-that-gates-nothing-2026-08-02). **Ticket 5 of N done and verified — `company` + `institution`, the two types the ticket-2 survey recorded as BLOCKED, and a leak on three surfaces.** **The catalog decision: a `Profile` arm, the sibling of `Ledger`.** Both are SIDECARS PK'd to `tenant_organizations` (M41 / D-UnifiedOrgGraph), so their rows' token is `organization` and `facet.Register` refused them; `Ledger` could not absorb them (capped at one, `audit` holds it, and these rows DO have a token — just not theirs alone). Two things make it a different kind of escape: it is **UNCAPPED**, because a sidecar-on-organization is a pattern rather than an exception, and it is **CHECKABLE** — a ledger's "these rows have no token" is written down nowhere and must be argued, while a profile's claim is a fact the DDL records, so the guard parses the migrations and asserts the profile table's primary key REFERENCES the profiled token's table. That is what replaces the cap; a companion guard pins that every profile hangs off the SAME parent today, which is not hypothetical (`religion_org_profiles` is the same shape keyed to `tenant_units`). Rejected and recorded: a domain-discriminated `organization` (the facets would bind to `listOrganizations`, leaving the endpoints that actually serve these rows unfilterable) and new RID types for the profiles (a migration re-keying two sidecars and every table referencing `institution_id`). **AND A LEAK: `listCompanies` and `listInstitutions` applied NO visibility gate at all**, since M21 and M20 — both join `tenant_organizations` and never looked at `visibility`, so a caller holding `company.read`/`education.read` was handed the shadow organizations `listOrganizations` trims. The `getOrganization` leak in a worse form (a whole list, not one RID at a time), in the two modules that inherited its rows; the point reads leaked the same rows, and unified SEARCH was a third door (both registered under the catalog IDENTITY scope). All three now route through one helper per module over `pep.FilterVisibleOrgs`, plus a new `scope.NewOrgScope`; a gated-out row is `NotFound`, **never 403**, because `shadow` hides EXISTENCE. **The search door had no guard, and breaking it is what showed that** — six of seven breaks went red immediately, and reverting the search scope left the whole suite GREEN, because the per-module transport guards cannot see a composition-root literal. `cmd/oikumenea/searchscope_test.go` now asserts by AST which scope each provider takes and refuses a new provider that is not listed, so `catalog` must be CHOSEN rather than inherited from the line above. **FOUR aggregate arms per type** (the person shape, not organization's two): a profile has BOTH a visibility gate and an R-21 search twin, so the square is {plain, search} × {admin, scoped}, and the scoped arms carry the DERIVED org reach — unit's own predicate would compile and match nothing. **`industryClass` could have taken `NonPartitioning` and does not**, confined to the PRIMARY assignment so it partitions honestly and answers the question the chart is read for (ticket 3's rule: an exemption being available is not a reason to take it). **`foundedOn` is the first YEAR-grain facet, and the live run taught it its limit**: the console rendered **356 bars for 8 institutions** because `yearSpan` densified 1661–2016 the way `monthSpan` densifies a fiscal year, every empty bar a live link — a dense axis exists to show a GAP, and past a screenful it shows nothing else. It now gives up past 60 years (the visual budget the ~54-bar vehicle fleet-age chart already uses) and returns the populated years: **356 → 15 segments**, found by rendering it and by nothing else. Also: **three stale claims from ticket 4's follow-up corrected** (two Conjure docs and an integration-test header still described the pre-derivation rule — the same defect class the ticket is about); the seeder gained a 16-row profile spread on the create-if-absent REFRESH path, and its positional shadow rule ("from the 4th code onward") was replaced with explicit codes after sixteen new orgs would have made nineteen of twenty-one shadow, silently; and the pii sweep came back DIRTY again (migration 0021 classifies six lifecycle columns both sidecars left alone). **Guards: 8/8 hand-broken and confirmed red. Verified live** (three subjects — admin, an insider with reach into the shadow company's unit, an outsider with reach into a public org only): totals 10/10 and 8/8 vs exhaustive paging, **44/44** admin buckets and **42/42** scoped landing on exactly the rows they counted, **10/10** distributions summing to total, and the headline differential — admin 10/8, insider 10/7, outsider 9/7, **chart and list agreeing on every row for every subject**, with the insider seeing the shadow company they reach and still 404-ing on the shadow institution. Search: `privatbank` 1/1/0 hits, `donetsk` 1/0/0. Numbers in [review-2026-07](architecture/review-2026-07.md#m58-ticket-5--the-sidecar-profiles-and-a-list-that-never-gated-2026-08-03). **Ticket 6 of N done and verified — `location` + `link__has_role`, the last two the board named, and neither had a LIST to declare facets over.** `listLocations` was a three-way switch (text / radius / bounding box) returning `Location:QueryWindowRequired` when given none of them; `listAssignments` required exactly one of `subjectPersonId`/`targetUnitId` — two endpoints wearing one name. Both now browse, and both errors are **vestigial rather than removed** (the contract is expand-only). **The finding is that the reach trim every other scoped list uses would have made this endpoint WIDER:** `authz_readable_units` asks whether the subject holds ANY `'%.read'` code on a unit, which is right where the endpoint has already checked its own read code — but `listAssignments`' per-unit arm has always demanded `assignment.read` on that unit specifically, and generic read-reach is a strict superset of it. Borrowing the family would have handed callers grants the arm it replaced refuses them, and nothing at a call site would show it: the two functions differ by ONE argument, read alike, and the wrong one compiles, passes every other guard and returns MORE rows. Migration 0023 adds permission-parameterised siblings of the 0017 trio comparing the code by **equality** — a LIKE-pattern argument would make `'%'` expressible by a typo — and leaves the 0017 trio untouched, per its own comment that its plans are measured. **Measured live the gap is real:** the insider's `assignment.read` reach is **40 units / 30 grants** against **41 / 31** generic, one unit reachable only through a role carrying `.read` codes but not `assignment.read`. **Two gates moved with it, both narrower:** the `targetUnitId` arm now returns an empty page rather than a 403 where the caller cannot reach the unit (identical rows, disclosing less), and the `subjectPersonId` arm — which gated on `assignment.read` held ANYWHERE and applied **no trim at all**, so one grant anywhere enumerated any person's authority instance-wide — is trimmed like everything else. **`ClassWindow` is the one new kernel concept:** a radius or a box is a predicate over the listed table, so it selects a subset of the same population and the dashboard HONOURS it; `ClassTraversal` says "no aggregate counts this" (right for a tree walk, exactly wrong here) and `ClassSearch` says "a separate plan shape that ships on stats" (only the second half applies). The class ships on `/stats`, is grouped by nothing, and is pinned to contract type `double` — which is what stops it becoming a place to hide an arg that could have been bucketed. **`location` ships FOUR aggregate arms whose axis is the listing MODE rather than visibility** (it has no owner and no shadow bit, so there is no scoped arm at all): four plans, four aggregate twins, because a nullable spatial predicate is not GiST-served — and the mode PRECEDENCE is resolved ONCE, in a shared filter builder both surfaces call, which is structurally stronger than a guard comparing two copies. **`subjectPersonId` is a fifth facet the tranche table never classified** (already an arg, `pii:basic` exactly like `targetUnitId`, and "who holds the most grants" is a real question) — the catalog corrected rather than worked around. **A drift guard was blind because its INPUT was stale:** regenerating `pkg/facet/args_gen.go` turned two arg guards red on `unit`, whose `query` arg (migration 0022's unit-picker fix) had never been classified — the guard was fine, nothing had regenerated the mirror it reads. Also: the pii sweep came back **DIRTY** a fifth time in six tickets (`type_id`, a FACETED column, plus six address parts, all unclassified while `raw_address` and the `search_text` generated from them were not); `atlas migrate lint` **could not run** (v0.38+ gates it behind Atlas Pro — the migration is additive by construction and applied cleanly, but that gate was not executed); and `assignment.graphId` renders three bars all reading "Command", since per-org graphs are distinct rows sharing a name — correct, clickable, and hard to read, recorded as an open seam rather than patched, because the fix is org-qualified labels across every ref labeler. **Guards: 12 tests across 5 files, 12/12 hand-broken and confirmed red** (one break had to be redone — hardcoding the admin flag left a variable unused, so the package failed to BUILD, which is not the same as a guard going red). **Verified live** (three subjects): **94/94** admin buckets and **86/86** insider landing on exactly the rows they counted, every distribution summing to total in every mode, the window narrowing 40 → 11 → 0 → 12 with chart and list agreeing at each step, the `graphId` `(unknown)` bucket equal to the `scope=unit` count (24 = 24) in the data rather than only in the DDL, 31 active / 6 revoked seeded with no revoked row in any page or bucket, **54 console segments** all resolving, and dashboards in 10–16 ms. Numbers in [review-2026-07](architecture/review-2026-07.md#m58-ticket-6--the-two-types-with-no-list-and-a-reach-that-was-one-argument-too-wide-2026-08-06). **Ticket 7 of N done and verified — `enrollment` (`link__studied_at`), the LAST type of the tranche, and with it M58 is COMPLETE.** Like the two before it, it had no list to declare facets over: `GET /persons/{personId}/enrollments` is one person at a time, so the register could be interrogated and never described; `GET /enrollments` is the browse mode and the per-person operation keeps its path as `listPersonEnrollments`. **The kernel finding is that a ref facet over a closed catalog is a SCALE, and topN destroys it:** `degreeLevelId` points at the nine migration-seeded ISCED 2011 levels, and sorted by frequency that chart reads "Bachelor, Doctoral, Master" — a ranking presented as a scale. New **`facet.StrategyCatalog`** orders by the referenced catalog's own ordinal, emits **every** level including the empty ones (on a scale an absent level is information) and has no `(other)` tail (a closed catalog has no tail to collapse); it is the ref analogue of `StrategyIdentity`, the rule `KindEnum.Values` has always carried, reaching the case where the ordered set lives in a table rather than a CHECK. Like `Profile` and unlike `Ledger` its claim is **CHECKABLE**, so a guard parses the migrations and asserts the facet's column really FK-references the named catalog and that the ordinal is really a column of it — without that a facet could name an unrelated table, LEFT JOIN it, return all zeroes and render. **Hand-breaking is what showed the first ordering guard was worthless:** its fixture used counts that happened to DESCEND with the ISCED level, so reverting the sort to count order left it green; the fixture now makes the two orders disagree and three tests go red. **AND A LEAK ON NINE ENDPOINTS: education applied the holder read scope NOWHERE.** D-PersonReadScope has required a person's data to be readable only through that person's read scope since M0 and only `document` implemented it — every person-binding read here (enrollments, dorm stays, education appointments and the six reference-layer bindings) gated `education.read` ANYWHERE and returned the rows, so ONE grant anywhere enumerated any person's education history instance-wide, since M20. All nine now probe one `holderReadable` helper and answer an unreadable holder with an EMPTY list, never a 403 (a permission error would confirm the person exists). The guard is structural — the gate lives per handler in the transport, where an application test has nothing to lose — and is held against the CONTRACT rather than itself: a tenth `GET /persons/{personId}/…` operation not in the list fails, which is the check that would have caught the original nine. **The bug the LIVE run found and nothing else could:** the list shipped reading the bare pool, and although `person_education_enrollments` carries no RLS policy its holder semi-join probes `membership_memberships`, which is FORCE RLS — unpinned, the `app.*` GUCs are unset, `authz_unit_in_reach` matches nothing, and the endpoint answers **200 with zero rows to a caller entitled to fourteen**, which reads as "no results" rather than as a fault. The integration suite cannot see it (superuser, RLS bypassed) and the guard that exists for exactly this had simply never been pointed at education; the module is in its table now and reverting the fix turns it red. Also: **`scripts/gen-action-params.sh` had been unable to run since ticket 6** — `location`'s comment sat between `Type:` and `Module:`, which the strict binding regex requires adjacent, so 17 declared types matched 16 and the generator exited; the committed mirror happened to be correct, so nothing went red and a generator that cannot run looked exactly like one with nothing to do. **Two non-defects recorded:** the ticket-2 survey's `programId` defect DOES NOT EXIST (a real FK since M20, added by an `ALTER` ~750 lines below the `CREATE TABLE` — the `vehicle.color` trap again; only `startedOn` was fictitious), and the trim here correctly takes the GENERIC `'%.read'` family rather than ticket 6's `_with` trio, because "may I read this PERSON" is the D-PersonReadScope projection and generic by definition. The pii sweep came back **DIRTY** a sixth time in seven tickets (`status` and `effective_from`, both faceted, unclassified since M20 — migration 0024), and `atlas migrate lint` **could not run** (Atlas Pro gate). The seeder wrote every enrollment identically, so four of seven facets had nothing to describe; the spread populates SIX of the nine ISCED levels on purpose, since a fully populated scale would let a topN regression pass. **Guards: 10 hand-broken and confirmed red. Verified live** (three subjects): totals **43/43** admin and **14/14** insider vs exhaustive paging, **41/41** buckets landing on exactly the rows they counted on each arm, **7/7** distributions summing to total, the differential admin 43 / insider 14 / outsider 0 with chart and list agreeing on every row, the per-person gate returning 0 to the insider for an unreachable holder and 1 for a reachable one, all **9** ISCED levels rendered in scale order with the three EMPTY ones first (where count order would put them last), **42/42** console segments resolving, dashboards in 73–123 ms. Numbers in [review-2026-07](architecture/review-2026-07.md#m58-ticket-7--the-scale-that-is-not-a-ranking-and-a-read-scope-nine-endpoints-never-applied-2026-08-06). **M58 is COMPLETE**: every listable object type in the catalog now carries facets, a stats endpoint, a console dashboard and an ontology-registry entry. Depends on M57 |
| **M59** | ✅ | ✅ | ✅ | ➖ | ✅ | ✅ | **ticket 1 of N — verified.** The cross-module chart and the first gated facets ([facets.md](architecture/facets.md), [D-ObjectFacets](architecture/decisions.md) amended three times). **(a) Headcount by unit, catalogued at M57 and undrawn for two milestones:** the blocker was a contract gap (`membershipStats` had no `org` arg, so an org-scoped dashboard would have mixed organizations), and the seam's claim that it also needed "a facet whose Table is not the listed table, which the catalog does not do today" was **already false when written** — three such facets have existed since M56. The `org` facet reads `authz_unit_org`, the RLS-**exempt** M55 projection, and NOT `tenant_units`, which is RLS-FORCED and would have answered a confident zero on an unpinned connection (the ticket-7 trap, avoided by design this time and pinned by a test). `ChartDef.source` is the console half: one extra request, buckets labelled by the SOURCE type, segments linking to the MEMBERSHIP list. **(b) Rule 2's first live cases, and it was a defect before it was a feature:** no facet had ever carried a `ReadPermission`, so the omission branch ran only against fixtures — and `vehicle.registrationCountry` had shipped in M58 ticket 3 grouping a table whose own endpoints require `vehicle.registration.read`, letting a plain `vehicle.read` caller derive registrations one value at a time. **The rule settled: a facet inherits the code of the SURFACE it reads, not the tier of the column it names.** `account.holderKind` is the born-gated twin, confined to the active PRIMARY holder by a partial UNIQUE so it partitions. **(c) The list side rule 2 never had:** a gated filter arg is **refused (403)**, not omitted — honouring it makes an oracle, ignoring it fails OPEN, and `facets=` names a dimension where a filter names a value. **(d) A guard blind spot found in passing:** the DDL parser's `COMMENT ON COLUMN` regex required a single space, so 11 aligned classifications were invisible to it — none `pii:special` today, meaning the contrapositive sweep was correct by luck. Fixed and pinned. New guards: cross-source charts (4 live-negative cases), `FilterReadCodes`, gated-code validity, the gated-surface rule, per-handler structural gates in vehicle + finance, and integration suites for both new facets. **Live run** (`oik serve` + the console's standalone build on the seed-demo dev DB): the org distribution sums exactly (94+7+0 = 101 with resolved org labels); the unit dashboard renders 16 headcount bars, every one linking to the MEMBERSHIP list with `org`+`unitId` and none to the unit list; the bar that says 3 lands on a list of 3 through the console's own BFF passthrough (the inverse checked by COUNTING, per M57 ticket 3); and a purpose-seeded subject holding `vehicle.read` WITHOUT `vehicle.registration.read` got the facet absent from `/stats` (200, and still 200 when it asked for the facet by name — never a zeroed bucket, never a 403) and **403 naming `vehicle.registration.read`** on the filter arg, while an entitled caller got 200 on both. Same pair proven for `finance.holder.read`. Incidental: `make sdk` had been stale on `main` since M58 ticket 7 (education's enrollment endpoints were never regenerated) — regenerated here |
...
(first-party login security log) remains at the **designed** gate. **M38 is verified** (criminal/arrest/court
records, D-LegalRecords). **M39** (compensation/payroll) is the one remaining **deferred stub** — no
decision/module doc yet, so it sits *before* the decided gate (its stage-board row is `⬜`). **M16 is verified** as the **hermenea** companion service (D-Hermenea
...
**Status: planned (designed).** A ten-milestone cluster (M29–M38) + one deferred stub (M39)
...
- **Excluded / deferred:** criminal/arrest/court records (6.1–6.3) → **M38** (built, D-LegalRecords).
...
## M37 — Login security log ✅ verified

**Goal.** First-party login/IP security telemetry on the federation seam. Binding via
**D-LoginSecurityLog** (+ its 2026-07-21 as-built note).

- **Delivers:** `account_login_events` (account Object `9,1,4`, migration `0043`: `account_id`,
  `context ∈ {login,activity,registration}`, `ip`, `first_seen_at`/`last_seen_at`, `occurrence_count`,
  + the deferred `resolved_country`/`resolved_isp`/`is_vpn`/`is_tor` intel overlay & `user_agent`;
  `pii:contact`, retention-bounded, purge-erased), recorded by the existing OIDC/JWKS validation
  middleware on the token-validation path — **not** OSINT enrichment, **not** stored credentials
  (L-AuthzOnly holds).
- **As built** (two user-agreed build decisions): **(1) bounded, not a firehose** — the emit is
  **deduped** to one row per `(account, context, ip)` per window (default 1h), off the critical path
  (detached goroutine) + best-effort, service principals excluded; **(2) client IP through the facade**
  — console-bff forwards a single authoritative `X-Forwarded-For`, trusted only under
  `login-security.trust-forwarded-for` (a narrow **D-HeadlessTopology amendment**). IP-intelligence is a
  deferred `IPIntel` no-op seam; a read endpoint gates on the instance-scope `account.security-log.read`.
- **Implements:** D-LoginSecurityLog (extends L-AuthzOnly). See
  [identity-federation](modules/identity-federation.md).
- **Exit (met):** a validated inbound token records a (deduped) login event; the retention sweep prunes
  old events; person purge erases the subject's events. Verified by unit (`clientIP`) + integration
  (dedup/window/erase/sweep on `oikumenea_test`) + the drift-guard baseline bump; the live HTTP e2e is
  deferred to the docker stack (host-binary boot blocker — see the stage-board row).

## M38 — Criminal / arrest / court records

**Status: verified.** Binding via **[D-LegalRecords](architecture/roadmap-decisions.md#d-legalrecords--category-level-criminal--arrest--court-records-piispecial-gdpr-art-10-extends-d-specialpii-d-overlayfoundation-d-geo)**
(draft macro-category 6.1–6.3). One `person_legal_records` Object (`6,1,22`) in
[personsensitive](modules/personsensitive.md#data-model): `kind ∈ {criminal_conviction, arrest,
court_judgment}` + a **mandatory `disposition`** (arrest ≠ guilt); the category-level offence `detail`
is **envelope-encrypted** (`pii:special`, GDPR **Art. 10** — amends the draft's `pii:sensitive` tag),
NOT-NULL `legal_basis`, never inferred, crypto-erased on purge; **jurisdiction** → `geo_countries`
(D-Geo). **Expungement/sealing suppression**: `is_suppressed`/`suppressed_reason` mark a **retained**
record withheld from `person.legal-record.read` and revealed only to a holder of
`person.legal-record.read-suppressed` (in no base role) or an instance admin. **Distinct** from internal
`discipline-incentive` orders ([order](modules/order.md), DS-36). Jurisdiction-specific display rules
(Ban-the-Box, FCRA) are an **open seam** — the data hook lands, the rule engine does not.
...
## M51 — Service identities ✅ verified

- **Delivers:** the OIDC/JWKS middleware accepts **client-credentials** tokens and resolves them to
  **service principals** (a machine subject kind on the request context, `person | service`); an
  admin-managed `(issuer, subject) → principal` registry on identity-federation; flat
  **per-principal grants** `(permission_code, org_id)` on authorization; the audit `system` actor
  becomes attributable to a specific principal. The M16 `hermenea-importer` env-secret path survives
  as the minimal-install fallback — reshaped into a real seeded principal so there is one downstream
  path rather than a hard-coded exemption.
- **Implements:** D-ServiceIdentities (extends D-Hermenea — amends its "Why not (e)" — and
  L-AuthzOnly), **as amended during implementation**: per-principal grants replace the original
  "role assignments" wording, which did not typecheck against `pdp.go` (instance-scope codes are
  admin-only and roles never carry them, so a principal given a role could not import).
- **Boundary:** a principal has **no reach**. It gets no authority snapshot and no RLS connection
  pinning, so every person-shaped PEP path denies it. Machine access to RLS-protected,
  organization-owned data (scraped units, memberships, clergy, university staff) is the **RLS
  service arm — M55** (split out of D-ConnectorPlane during M53, which built the connector plane's
  reads only); the `org_id` column and its enforcement ship here so M55 retrofits nothing.
- **Exit (verified):** a client-credentials token from the local Keycloak resolves to a registered
  principal and imports (200) once granted, while the *same* token 401s before registration (the
  rejection logs `issuer`/`azp`/`subject` so an operator can register it); the audit row names the
  principal; the env-secret fallback still imports and now names its seeded principal; a machine is
  403 on RLS-consuming reads; revoking a grant denies immediately and disabling a principal 401s.

## M52 — Console facade (headless topology)

- **Delivers:** **console-bff** — session + user-token passthrough + console hosting, on the M27
  SDKs, zero privileged credentials, **no on-behalf-of**. Realized as the console's **own Next.js
  server tier**, which already met every one of those constraints, rather than a new deployable
  (D-HeadlessTopology *amended*; the compose service is renamed `web` → `console-bff`). Compose
  stops publishing oikumenea's ports — **both** `8443` and the `8444` management port, since
  `/status` and `/debug` bypass authentication. Also delivers the **service-principal admin
  console** (M51 deferred its UI here) and the console's first permission gating, `can()`.
...
- **Delivers:** the connector **registry** (`Connector`/`Source`/`SyncRun` Objects + RID service
  20, audited sync-run reporting); the narrow permission-gated **wiring API** (natural-key→RID
  resolution, reference-catalog reads, sync cursors — each surface its own instance-scope code);
  the M34 `watchlistclient` refactored into the generic typed **connector-call seam** (`internal/
  connectorcall`, deadline mandatory); hermenea registers itself and reports its runs; a
  **read-only console fleet view** (connectors → sources → runs, no trigger/edit controls).
- **Scope:** the connector plane's **reads and reporting only**. The **RLS service arm** (machine
  reach into RLS-protected, organization-owned data) was split out to **M55** — see
  D-ConnectorPlane's dated amendment; M53 gives a connector no reach beyond instance-wide reference
  data.
...
- **Exit (verified when):** hermenea appears in the registry with its sources and a completed
  ...
  (fixture) connector registers with zero core code changes; the console fleet view renders for an
  instance admin and is hidden from a non-admin.
...
- **Delivers:** the D-Pinax autoseeder generalized to scan **operator-mounted packs** beside the
  ...
  `i18n_locales` row via a new `locales` import objectType + translation overlays); the
  `modules.*.enabled` install-config map over the **six enrichment verticals**
  (finance/religion/vehicle/externalorg/company/education) — a disabled module hides its endpoints and
  makes its codes non-grantable, but its **schema still migrates**.
...
## M55 — RLS service arm (machine reach into org-owned data) ✅ verified

- **Why it exists:** the half of D-ServiceIdentities / D-ConnectorPlane that was **deferred out of
  M53** (see D-ConnectorPlane's 2026-07-21 amendment). M51 shipped the mechanism — a service
  principal, flat `(permission_code, org_id)` grants, the `org_id` column already carrying an
  organization dimension — but a machine still got **no reach** into RLS-protected, organization-owned
  data. M53 built the connector plane's instance-scope reads; M55 is the reach.
- **Delivers:** a **principal branch in `authz_unit_in_reach`** so an org-confined principal grant
  authorizes reads/writes on that organization's RLS-guarded rows (scraped units, memberships, clergy,
  university staff); org-owned import targets, with `RequireImport` passing a **real orgID** (optional
  envelope `orgId`); the authenticator now installs a **lazy RLS-scoped connection for a machine
  subject** (`ContextWithPrincipalAuthority` returns `RLSState{PrincipalID}`), so a principal is pinned
  under `app.principal_id` like a person is under `app.person_id` — while every person-shaped PEP path
  still denies it (the reach is bounded to what its org grant authorizes at the DB).
- **The central design problem** (why it is its own milestone, not an M53 sub-task): the RLS backstop
  function [`authz_unit_in_reach`](../migrations/0005_document_order_rls.sql) may read **only
  RLS-exempt tables** (`authz_*`, `tenant_graphs`, `tenant_unit_closure`) — reading a policy-guarded
  table from inside a policy recurses into that same policy. So a principal arm **cannot** simply join
  `tenant_units` to learn a unit's organization, even though `tenant_units.org_id` exists.
- **As built (migration `0042`, 2026-07-21).** A dedicated **RLS-exempt projection**
  `authz_unit_org(unit_id → org_id)`, trigger-maintained from `tenant_units` (`sync_unit_org` BEFORE
  INSERT/UPDATE), with the `unit_id` FK **`DEFERRABLE INITIALLY DEFERRED`** so the projection row can be
  written from the BEFORE trigger — before the parent `tenant_units` row lands — without an
  immediate-FK violation. A third GUC **`app.principal_id`** and `authz_principal_org_in_reach(org, wr)`
  (the org-direct grant primitive: `org_id IS NOT NULL AND revoked_at IS NULL AND (code LIKE '%.read')
  = NOT wr`, read live) feed a principal arm in `authz_unit_in_reach` that resolves a **child** table's
  unit → org through the projection. **`tenant_units`' own policy** instead gets the org-direct arm on
  the row's `org_id` column — the one case the projection can't serve, because a BEFORE-trigger write
  is not visible to the same statement's `WITH CHECK`; this is what lets a connector CREATE a
  brand-new, edgeless (not-yet-in-closure) unit. `org_id IS NULL` (instance-wide) grants confer **no**
  operational reach — a scraper's blast-radius boundary is its organization. The person + admin arms
  are byte-identical to `0011`, and the new arm is an empty probe when `app.principal_id` is unset, so
  no person hot path regresses.
- **Implements:** D-ServiceIdentities + D-ConnectorPlane (the deferred RLS arm). Requires M51; builds
  on M53. Additive / expand-only.
- **Exit (met):** an org-confined principal reads and writes that organization's RLS-guarded rows and
  **only** that organization's (a cross-org read/write is denied at the DB, not merely the PEP);
  `RequireImport` accepts an org-scoped import under a matching org grant and rejects it under a
  foreign one; no person-shaped hot path regresses (the person arm is unchanged; reach/RLS suites
  green). Verified by `internal/platform/db/rls_service_arm_integration_test.go` (incl. brand-new unit
  creation via the projection + cross-org DB denial + live revocation) and a live host-binary e2e on
  the restricted role (import `orgId=O` 200 / foreign `orgId=P` 403, person surface 403, revoke →
  403 immediately). **Note:** the machine write path over HTTP is the **import endpoint**
...
- **Delivers:**
  ...
  - **The `Facet` declaration** per object type — ✅ **landed (ticket 2)**, for `person` and `unit`.
    `{key, kind ∈ enum|ref|date-range|bool|numeric-range, table, column, readPermission, buckets}` in
    the shape `internal/links`'s `Descriptor` established, living in **`pkg/facet`** so the catalog,
    the IR-derived mirror and the drift test share a package (the `pkg/action` arrangement — the one
    precedent that already pairs a hand-written catalog with a generator). Bucket strategies are
    declared now and consumed by M57.
  - **Typed Conjure query args** — ✅ **landed (ticket 2)** on `listPersons` (`sex`, `status`,
    `birthdateFrom`/`To`, `countryOfBirth`, `rankId`, `unitId`, `graph`, `hasAccount`) and `listUnits`
    (`visibility`, `state`, `pdpScoped`, joining the four it already shipped), copying the
  ...
  - **An IR-derived drift guard** — ✅ **landed (ticket 2)**. `tools/genfacetargs` projects each bound
    list endpoint's query args out of the Conjure IR into `pkg/facet/args_gen.go` (driven by the
    existing `scripts/gen-action-params.sh`, one IR dump for all three mirrors), and the guard fails
    the build in BOTH directions. The second direction is the one that decays, so a non-facet arg
    must carry a *classified* role — `paging` accepts only `pageSize`/`pageToken` with their contract
    types; `search`/`traversal` must name a query that really exists in the module's `queries/*.sql`
    — which is what stops the mechanism becoming an allowlist that hides the drift it exists to
    catch. The script also gained **`--verify`**, because a stale mirror would let the guard validate
    yesterday's contract and pass; that closed the same latent hole for `pkg/action`.
  - **Build-time enforcement of D-ObjectFacets rules 1–2** — ✅ **landed (ticket 2)**, parsed out of
    `migrations/*.sql` with no database (the `rewrap_registry_test.go` technique): a facet may name
    only a plaintext column, one above `pii:basic` must inherit its field's own read code, a nullable
    column may not omit its `(unknown)` bucket, plus a contrapositive sweep asserting **no**
    `pii:special` column anywhere is faceted.
  - **The missing top-level list endpoints** `listOrders`, `listDocuments`, `listMemberships` (today
    only per-unit / per-person paths exist).
  - **Console** — ✅ **landed (ticket 4).** `filters?: FilterDef[]` on `ObjectTypeDef` (sibling to the
    existing `ColumnDef`/`PropertyDef`/`LinkDef`/`ActionDef`), a filter bar on `/explore/[type]`, and
    **all filter state in the URL** — retiring the four hand-concatenated query strings, the two
    `<form method="get">` that clobbered each other, and `DataTable`'s `useState` quick-filter for
    every type the backend can actually narrow. `order`, `document` and a new `link__member_of` entry
    gained the `list:` defs ticket 3's endpoints had no console surface for, and the TS SDK was
    regenerated (it shipped none of the three).
...
- **Ticket-2 status against that exit:** person + unit facets round-trip on both the admin and
  read-scope paths (integration, incl. an `admin ∩ reach` differential and a one-row-at-a-time paging
  drain); both guard directions were hand-broken and confirmed red; the scale dataset gained facet
  distributions (`seed-scale -enrich`) and the plan guard asserts no `Seq Scan` on `person_persons` —
  numbers and the three findings that changed code in
  [review-2026-07](architecture/review-2026-07.md#m56-ticket-2--person-facet-filters-2026-07-28).
  Membership's filtered paths land with ticket 3 (the endpoint does not exist yet); the console is
  ticket 4.
- **Ticket-4 status against that exit:** every declared facet round-trips as a URL-borne list filter
  on all five types (live, `oik serve` + `next dev` against the `scripts/seed-demo` DB); the console
  guard was hand-broken three ways and confirmed red each time; the filter bar's state survives a
  refresh, a new tab and the Table↔Tree toggle; `tsc --noEmit` and `next build` are clean. The
  whitelist ("a junk param never reaches the API") is asserted at the module level rather than on the
  wire, because witchcraft's request log records the path but not the query string.
...
- **Delivers:**
  - **`GET /<module>/v1/stats/<collection>`** — one endpoint per module, taking **exactly the same
    ...
    Backed by **static sqlc `GROUP BY` queries** — as built, one per (module, ARM) rather than per
    (module, facet): one materialized candidate CTE carrying the list's filter block verbatim, then one
    `UNION ALL` branch per facet gated on a `want_*` flag the planner SKIPS. No dynamic SQL either way
    (D-ObjectFacets as-built note).
  - **Visibility folded into the count.** Person-scoped counts reuse the reach semi-join already
    proven at scale — as built, the SET form ALONE: with no `LIMIT` to terminate early the sparse/dense
    dispatch loses its reason to exist, and the measurement agreed at every reach size; unit-scoped
    counts fold the shadow gate **into SQL** — `gateUnits` trims
...
- **Implements:** D-ObjectFacets (stats half), D-ConsoleDashboards (dashboard half). Requires M56.
  Additive; no schema beyond any index the measurement demands (none were: the aggregates reuse
  migration `0017`'s reach functions).
- **Ticket split**, mirroring M56's: **1** the `pkg/stats` kernel + the `person`/`unit` stats endpoints
  + the differential and scale harnesses — **done**; **2** `membership` / `order` / `document` stats —
  **done** (the backend half of M57 is complete; every registered type has a dashboard endpoint);
  **3** the `@visx/*` primitives, `dashboard?: DashboardDef` and `?view=dashboard` — **done** (server
  SVG, no client JS; the bucket→filter inverse in `lib/ontology/buckets.ts`; the fourth drift guard);
  **4** the live end-to-end run — **done** (all five dashboards, and the click-through inverse checked
  exhaustively against the lists its segments link to).
...
## M59 — The cross-module chart & the first gated facets

**Goal.** Close the two things the facet program (M56 → M57 → M58) recorded as open and did not
build. Both were catalogued, both were assigned to "M58", and M58 closed without either.

- **Delivers:**
  - **`link__member_of.org`** — the membership row carries no `org_id`, so the facet matches the
    organization of its UNIT through **`authz_unit_org`**, the RLS-exempt trigger-maintained
    projection M55 built. Deliberately not `tenant_units`: that table is RLS-FORCED, and a semi-join
    into it from this module would be trimmed by a policy written for unit reads — a confident zero to
    an entitled caller on an unpinned connection, the trap M58 ticket 7 shipped and the live run
    caught. Ships on `listMemberships` and `membershipStats`, in all five SQL arms.
  - **`ChartDef.source`** — a dashboard chart drawn from ANOTHER module's stats endpoint, carrying an
    **allowlist** of the host's params. With it, the unit dashboard's **headcount by unit** (M57
    component ⑤) is finally drawn; its bars link to the membership list, because the rows behind the
    number are memberships.
  - **D-ObjectFacets rule 2's first live cases** — `vehicle.registrationCountry` (retrofitted;
    `vehicle.registration.read`) and `account.holderKind` (born gated; `finance.holder.read`), the
    first facets in the vocabulary carrying a `ReadPermission` at all.
  - **The list side of rule 2** — `ObjectType.FilterReadCodes` plus a per-handler gate in vehicle and
    finance: a gated facet's filter arg is **refused (403)**, where its distribution is omitted.
- **What it fixed, not just added.** `vehicle.registrationCountry` shipped ungated in M58 ticket 3
  over `vehicle_registrations`, whose own endpoints require `vehicle.registration.read` from a
  separate base role. A caller with plain `vehicle.read` could group the fleet by registration country
  and filter the list by it — recovering, one value at a time, what those endpoints refuse to return.
  **The rule that settles it: a facet inherits the read code of the SURFACE it reads, not the tier of
  the column it names.** Rule 2's mandatory arm keys on the column tier, which is necessary and not
  sufficient — a cross-table facet discloses a *relationship*, and a relationship has its own code
  (D-LinkPermissions).
- **Two stale claims corrected in the docs.** The headcount seam said the chart needed "a facet whose
  Table is not the listed table, which the catalog does not do today" — three such facets have existed
  since M56 (`person.rankId`, `person.unitId`, `person.hasAccount`), and `Facet.Table`'s own comment
  says it "is NOT always" the listed table. And rule 2's nominated first example
  (`person_health_records.kind`) is `pii:special` in the DDL, so rule **1** forbids a facet over it —
  it could never have been the case that exercised rule 2.
- **A guard blind spot, found in passing.** `plaintext_test.go`'s `COMMENT ON COLUMN` regex required a
  single space before `IS`, so 11 aligned classifications were invisible to it. An unparsed comment
  reads as "no classification", so an aligned `pii:special` column would have been invisible to the
  contrapositive sweep while its non-vacuity floor still passed on the ~500 columns it did see. None
  is `pii:special` today: the sweep was correct by luck rather than by construction.
- **Implements:** D-ObjectFacets (amended three times: the list side of rule 2, the surface-not-tier
  rule, and cross-source charts). Requires M58. **Additive; no migration** — the projection, the
  partial UNIQUE and every read code it uses already existed.
- **Exit (verified when):** the `org` facet selects identically on both list arms and its stats total
  equals exhaustive paging under the same filter (asserted, and proven non-vacuous by breaking the
  predicate); the org distribution partitions; a caller **with** `vehicle.read` but **without**
  `vehicle.registration.read` gets the facet ABSENT from `/stats` (never zeroed, never a 403) and a
  **403** on the filter arg, while a caller with both gets identical totals; `account.holderKind`
  counts a jointly held account once; and a **live end-to-end** run opens the unit dashboard under an
  org filter, clicks a headcount bar, and lands on that unit's membership list.
