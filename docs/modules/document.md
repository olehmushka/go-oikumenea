...
| `GET /documents` | **Top-level facet-filtered list** across every readable holder (M56 / D-ObjectFacets). Filters: `typeId`, `status`, `issuingCountryId`, `issuedOnFrom`/`To`, `expiresOnFrom`/`To`. Metadata only — `number`/`issuer` and the `attributes` bag are readable but NOT filterable (D-ObjectFacets rule 1) | `document.read` **anywhere**, then HOLDER read-scope (D-PersonReadScope) folded INTO the SQL as a membership semi-join — `document_documents` has no unit column and no RLS policy, so visibility runs through the holder. An unreadable holder's documents are ABSENT from the result rather than post-filtered (R-06) |
| `GET /stats/documents` | Facet distributions over the **same** filter args + an optional `facets` CSV (M57; [facets catalog](../architecture/facets.md)). `totalCount` equals what exhaustively paging `GET /documents` returns under the same filters. `expiresOn`'s `(unknown)` bucket is the NO-EXPIRY (permanent document) population — a real set, not missing data | `document.read` **anywhere**, then the same HOLDER read-scope semi-join folded into the SQL, so an unreadable holder's documents are never counted (one plan shape: measured, the materialized reach set wins here too) |
