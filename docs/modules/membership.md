...
| `GET /stats/memberships` | Facet distributions over the **same** filter args + an optional `facets` CSV (M57; [facets catalog](../architecture/facets.md)): `totalCount` + per-facet buckets, counted INSIDE the reach predicate. `totalCount` equals what exhaustively paging `GET /memberships` with the same filters returns — a differential test asserts it, and the absence of an implicit status default is what lets the total agree with its own status distribution. `positionId`'s `(unknown)` bucket is the memberships with no billet | `membership.read` **anywhere**, then the same reach folded into the SQL (ONE plan shape — an aggregate has no `LIMIT` for the reach set to spoil) |
