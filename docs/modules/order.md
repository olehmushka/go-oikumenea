...
| `GET /stats/orders` | Facet distributions over the **same** filter args + an optional `facets` CSV (M57; [facets catalog](../architecture/facets.md)). `totalCount` equals what exhaustively paging `GET /orders` returns under the same filters. Two shapes worth knowing: `orderTypeId` counts order ITEMS' types, so an order with two effects lands in two buckets and that distribution does NOT sum to `totalCount`; and `issuedOn`'s `(unknown)` bucket is the DRAFT backlog | `order.read` **anywhere**, then reach on the ISSUING unit folded into the SQL (one plan shape) |
