...
| `LoginEvent` *(M37)* | [identity-federation](modules/identity-federation.md) | no | append-with-dedup; hard-erased on purge; retention-swept | `account_login_events` (`9,1,4`): a first-party login/IP security-log occurrence, `pii:contact`, fed from the token-validation seam. Bounded/deduped — one row per `(account, context, ip)` per window (`occurrence_count` + `last_seen_at`). The `resolved_*`/`is_vpn`/`is_tor` IP-intel overlay is a deferred seam (NULL until a resolver ships). Read on the instance-scope `account.security-log.read`. D-LoginSecurityLog |
