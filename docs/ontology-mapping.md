...
| `EducationInstitution` / `EducationUnit` / `EducationBuilding` / `EducationGroup` *(M20)* | [education](modules/education.md) | institution/unit/building/group `code` | soft-delete | external reference orgs (RID service 14); `EducationUnit` is a recursive per-institution tree (closure); buildings → `Location`; D-Education |
...
| `EDUCATION_UNIT_PARENT_OF` *(M20)* | `EducationUnit` → `EducationUnit` | [education](modules/education.md) | per institution; closure maintained | strict tree, a containment FK — *not* a reified Link (closure is `education_unit_closure`) |
