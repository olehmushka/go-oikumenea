...
func TestWatchlists(t *testing.T) {
	...
	// ---- CheckWatchlists without the seam wired => ErrWatchlistUnavailable ----
	p := newPerson(t, svc, "Ivan Screened")
	if _, err := svc.CheckWatchlists(ctx, p.ID); !errors.Is(err, domain.ErrWatchlistUnavailable) {
		t.Fatalf("expected ErrWatchlistUnavailable without a seam, got %v", err)
	}
	...
	svc.SetWatchlistLookup(fake)

	m, err := svc.CheckWatchlists(ctx, p.ID)
	if err != nil {
	...
	got, ok, err := svc.GetWatchlistMatch(ctx, p.ID)
	...
	if _, err := svc.UpsertGovernmentPosition(ctx, domain.GovernmentPosition{
		PersonID: p.ID, Title: "Senator", Body: "Parliament",
	}); err != nil {
		t.Fatalf("add government position: %v", err)
	}
	m2, err := svc.CheckWatchlists(ctx, p.ID)
	if err != nil {
	...
	x, err := svc.UpsertRegulatorySanction(ctx, domain.RegulatorySanction{
	...
	if _, err := svc.UpsertRegulatorySanction(ctx, domain.RegulatorySanction{
		PersonID: p.ID, Regulator: "FCA", Amount: fptr(10), // no currency
	}); err == nil {
		t.Fatal("expected amount-without-currency to be rejected")
	}
	xs, err := svc.ListRegulatorySanctions(ctx, p.ID)
	if err != nil || len(xs) != 1 {
		t.Fatalf("list regulatory sanctions: n=%d err=%v", len(xs), err)
	}
	if err := svc.DeleteRegulatorySanction(ctx, p.ID, x.ID); err != nil {
		t.Fatalf("delete regulatory sanction: %v", err)
	}
	xs, _ = svc.ListRegulatorySanctions(ctx, p.ID)
	if len(xs) != 0 {
...
func TestWatchlistMerge(t *testing.T) {
	ctx := context.Background()
	svc, pool := newService(t, 720)
	svc.SetWatchlistLookup(&fakeWatchlist{result: domain.WatchlistScreenResult{OnList: false, Lists: []string{}}})
	...
	if _, err := svc.CheckWatchlists(ctx, stub.ID); err != nil {
	...
	xs, err := svc.ListRegulatorySanctions(ctx, canonical.ID)
