...
// sealed (Seal) on write, decrypted (Open) on read, and crypto-erased on purge. (Name aliases stay with
// the person core — aliases are still names — R-09.)
package application
...
func (s *Service) UpsertPhysicalDescription(ctx context.Context, d domain.PhysicalDescription) (domain.PhysicalDescription, error) {
	...
	err := s.inTx(ctx, func(tx pgx.Tx) error {
		repo := s.newRepo(tx)
		if err := repo.PersonExists(ctx, d.PersonID); err != nil {
			return err
		}
...
func (s *Service) UpsertDistinguishingMark(ctx context.Context, m domain.DistinguishingMark) (domain.DistinguishingMark, error) {
	...
	err := s.inTx(ctx, func(tx pgx.Tx) error {
		repo := s.newRepo(tx)
		if err := repo.PersonExists(ctx, m.PersonID); err != nil {
			return err
		}
...
func (s *Service) ethnicityNamesByCode(ctx context.Context, repo domain.SensitiveRepository) (map[string]string, error) {
