...
func render(resolved map[string]endpoint) []byte {
	...
	var b bytes.Buffer
	// The SPDX header is emitted here, not stamped afterwards: the licensing pass added it to the
	// committed file by hand, so regeneration silently stripped it and left the output permanently
	// diverged from the tree. Emitting it keeps regeneration idempotent, which is what
	// `scripts/gen-action-params.sh --verify` needs to be meaningful.
