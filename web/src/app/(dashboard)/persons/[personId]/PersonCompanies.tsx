...
export function PersonCompaniesManager({ personId }: { personId: string }) {
  ...
  return (
    <div className="space-y-3 text-sm text-slate-700">
      {empty ? <p className="text-slate-400"><T>No company affiliations.</T></p> : null}
      {aff && aff.appointments.length > 0 ? (
        <Group title={tr("Employment")}>
          {aff.appointments.map((a) => <li key={a.id}>{a.positionTitle} · {a.companyName} · {a.status}{a.effectiveFrom ? ` · ${a.effectiveFrom}→${a.effectiveTo ?? tr("now")}` : ""}</li>)}
        </Group>
      ) : null}
      {aff && aff.foundings.length > 0 ? (
        <Group title={tr("Founded")}>{aff.foundings.map((f) => <li key={f.id}>{f.companyLabel || tail(f.companyId)}{f.foundedOn ? ` · ${f.foundedOn}` : ""}</li>)}</Group>
      ) : null}
      {aff && aff.shareholdings.length > 0 ? (
        <Group title={tr("Shareholdings")}>{aff.shareholdings.map((s) => <li key={s.id}>{s.companyLabel || tail(s.companyId)}{pct(s.stakePct)}</li>)}</Group>
      ) : null}
      {aff && aff.beneficiaryOf.length > 0 ? (
        <Group title={tr("Beneficial owner of")}>{aff.beneficiaryOf.map((b) => <li key={b.id}>{b.companyLabel || tail(b.companyId)}{pct(b.ultimatePct)}</li>)}</Group>
      ) : null}
      <p className="mt-1 text-xs text-slate-400"><T>Read-only. Company links are recorded from the Companies workspace.</T></p>
    </div>
  );
}
